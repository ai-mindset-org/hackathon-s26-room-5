...
RULES = [
    {"id": "персональные данные", "что": "имена, телефоны, почта и добавочные заменены плейсхолдерами",
     "источник": "боль заказчика: данные уходят наружу, персональное надо скрыть",
     "важность": "нарушение",
     "не_нарушение": "инвентарные номера (INV-0042), номера заказов и даты — не персональные данные"},
    {"id": "один человек — один плейсхолдер", "что": "повторное упоминание того же человека получает тот же номер, включая падежные формы",
     "источник": "боль заказчика: по разным плейсхолдерам одного человека не собрать обратно",
     "важность": "нарушение",
     "не_нарушение": "однофамильцы с разными именами — разные люди"},
    {"id": "обратный маппинг", "что": "прогон через mapping.json возвращает оригинал байт в байт",
     "источник": "боль заказчика: данные уходят наружу и должны вернуться обратно",
     "важность": "нарушение", "не_нарушение": "—"},
    {"id": "полнота", "что": "в обезличенном тексте не осталось почты и телефонов",
     "источник": "проверка самого себя после замены",
     "важность": "нарушение", "не_нарушение": "—"},
]

RUNTIME_ONLY = [
    "паспортные данные, адреса и номера счетов модуль не ищет — их шаблоны не заданы",
    "решение, считать ли конкретное имя персональными данными, остаётся за человеком: "
    "подпись автора под инструкцией и клиент в отчёте выглядят одинаково",
]
